...
class Server{
  public:
    ...
    void createSkinMask();
    void splitImage(const cv::Mat& inputIMG);
    void saveImg(cv::Mat img, std::string filename);
    void sendMessage(int connfd, char *format);
    void readVideoImgFromDisk(std::string filePath = "./../Video/frame%01d.jpg", int mode = cv::CAP_IMAGES, int length = 29, int start = 1);
  ...
  private : 
    ...
    cv::Mat m_left, m_right, m_skinMask;
